...
(function () {
    ...
    stdout("Generate private key (phpseclib)...\n");
    $privateKey = $generator->generatePrivateKey();
    $privateKeyPem = $privateKey->getPrivateKeyPem();
    stdout("OK\n");

    stdout("Export public key (phpseclib)...\n");
    $publicKey = $privateKey->getPublicKey();
    $publicKeyPem = $publicKey->getPublicKeyPem();
    stdout("OK\n");

    stdout("Load private key (phpseclib)...\n");
    $privateKey = new RSAPrivateKey($privateKeyPem, Utils::ENGINE_PHPSECLIB);
    stdout("OK\n");

    stdout("Load public key (phpseclib)...\n");
    $publicKey = new RSAPublicKey($publicKeyPem, Utils::ENGINE_PHPSECLIB);
    stdout("OK\n");
    ...
    foreach ($algs as $alg => $algOpenssl) {
        stdout("Create $alg signature (phpseclib)...\n");
        $signer = new RSASigner($privateKey, $alg);
        $signature = $signer->sign($data);
        stdout("OK\n");

        stdout("Verify $alg signature (phpseclib)...\n");
